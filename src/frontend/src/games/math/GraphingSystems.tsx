...
function genTask(diff: 1 | 2 | 3): GridTask {
  if (diff === 1) {
...
function genLineTracerQ(diff: 1 | 2 | 3): LineTracerQ {
...
interface DataTable {
  ...
  xLabel: string;
  yLabel: string;
  rows: { x: string | number; y: number }[];
  questions: { q: string; options: string[]; correctIdx: number }[];
}
...
export default function GraphingSystems({ config, onGameEnd }: Props) {
  ...
  const [task, setTask] = useState<GridTask>(() => genTask(config.difficulty));
  ...
  function gridToSvg(gx: number, gy: number) {
  ...
  function g1Submit() {
    const val =
      config.difficulty === 1
        ? g1Input.trim().toUpperCase()
        : Number.parseFloat(g1Input.trim());
    ...
    const ok =
      config.difficulty === 1
        ? val === task.answer
        : Math.abs((val as number) - (task.answer as number)) <= 0.15;
    ...
    setG1Feedback({
      msg: ok
        ? `Correct! ${task.answer}`
        : `Wrong. Answer: ${task.answer}. ${task.hint}`,
      ok,
    });
    const nextTask = taskIdx + 1;
    if (nextTask >= MAX_TASKS) {
      setTimeout(() => endGame(newScore, newCorrect, newTotal, true), 1000);
    } else {
      setTimeout(() => {
  ...
  const [g2Questions] = useState<LineTracerQ[]>(() =>
  ...
  const [g3TableIdx] = useState<number[]>(() =>
    Array.from({ length: ROUNDS }, (_, i) => i % DATA_TABLES.length),
  );
  const [g3QIdx, setG3QIdx] = useState(0);
  ...
  const g3Question = g3Table?.questions[g3QIdx];
  ...
  function g3Choose(optIdx: number) {
    ...
    setG3Feedback({
      ...
      msg: ok
        ? "Correct!"
        : `Wrong. Answer: ${g3Question.options[g3Question.correctIdx]}`,
    });
    setTimeout(() => {
          ...
          {g1Phase === "idle" && (
            ...
            >
              ...
              <p className="text-muted-foreground text-center max-w-sm">
                Plot points on coordinate grids and answer questions about
          ...
          {g1Phase === "playing" && (
            <div className="flex-1 flex flex-col gap-3">
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>
                  Task {taskIdx + 1}/{MAX_TASKS}
                </span>
                <span>
                  Score:{" "}
                  <span className="text-[#a855f7] font-bold">{score}</span>
                </span>
        ...
        <>
          {g2Phase === "idle" && (
            ...
            >
              ...
              <p className="text-muted-foreground text-center max-w-sm">
                Match equations to graph descriptions. Read the slope and
                y-intercept carefully.
              </p>
              <p className="text-xs text-muted-foreground">
                {TOTAL_LT} questions | Difficulty {config.difficulty}
              </p>
              <button
                type="button"
                onClick={() => {
          ...
          {g2Phase === "playing" && g2Questions[g2Idx] && (
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div
                    ...
                    style={{ fontFamily: "'Orbitron', sans-serif" }}
                  >
                    {g2Questions[g2Idx].equation}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Which description best matches this equation?
                  </p>
                  <div className="flex flex-col gap-2 w-full max-w-md">
                    {g2Questions[g2Idx].options.map((opt, i) => (
                      <button
                        key={i}
                        type="button"
                        onClick={() => g2Choose(i)}
                        ...
                        className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${
                          g2Feedback
                            ? i === g2Questions[g2Idx].correctIdx
                              ? "border-[#4ade80] bg-[#4ade80]/10 text-[#4ade80]"
                              : g2Chosen === i
                                ? "border-[#f43f5e] bg-[#f43f5e]/10 opacity-70"
                                : "border-border/30 opacity-50"
                            : "border-border hover:border-[#a855f7]/50 hover:bg-[#a855f7]/10"
                        }`}
                      >
                        {opt}
                      </button>
        ...
        <>
          {g3Phase === "idle" && (
            ...
            >
              ...
              <p className="text-muted-foreground text-center max-w-sm">
                Analyse data tables across {ROUNDS} rounds (5 questions each).
                Identify trends, correlations, and best graph types.
              </p>
              <button
                type="button"
                onClick={() => {
          ...
          {g3Phase === "playing" && g3Table && g3Question && (
            <div className="flex-1 flex flex-col gap-3">
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>
                  Round {g3Round + 1}/{ROUNDS} | Q {g3QIdx + 1}/5
                </span>
                <span>
                  Score:{" "}
                  <span className="text-[#a855f7] font-bold">{score}</span>
              ...
              <div className="rounded-xl border border-[#a855f7]/30 bg-card/40 p-3">
                <p className="text-xs font-bold text-[#a855f7] mb-2 uppercase tracking-widest">
                  {g3Table.title}
                </p>
                <div className="overflow-x-auto">
                  <table className="text-xs w-full">
                    <thead>
                      <tr>
                        <th className="text-left pr-4 pb-1 text-muted-foreground">
                          {g3Table.xLabel}
                        </th>
                    ...
                    <tbody>
                      {g3Table.rows.map((row, i) => (
                        <tr key={i}>
                          <td className="pr-4 py-0.5 font-mono text-foreground">
                            {row.x}
                          </td>
                          <td className="font-mono text-[#a855f7] font-bold">
                            {row.y}
                          </td>
                        </tr>
                      ))}
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <p className="text-sm font-bold">{g3Question.q}</p>
                  {g3Question.options.map((opt, i) => (
                    <button
                      key={i}
                      type="button"
                      onClick={() => g3Choose(i)}
                      ...
                      className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${
                        g3Feedback
                          ? i === g3Question.correctIdx
                            ? "border-[#4ade80] bg-[#4ade80]/10 text-[#4ade80]"
                            : g3Chosen === i
                              ? "border-[#f43f5e] opacity-70"
                              : "border-border/30 opacity-50"
                          : "border-border hover:border-[#a855f7]/50 hover:bg-[#a855f7]/10"
                      }`}
                    >
