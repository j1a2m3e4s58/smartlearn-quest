...
// ─── Game 1: Circuit Builder data ─────────────────────────────────────────────────
interface CircuitChallenge {
  description: string;
  components: string[];
  correctOrder: number[];
  circuitType: string;
  ohmQuestion?: { text: string; answers: string[]; correct: number };
}

const D1_CHALLENGES: CircuitChallenge[] = [
  {
  ...
  {
    description:
      "Wire: battery connects to switch, switch to motor, motor back to battery.",
...
const D2_CHALLENGES: CircuitChallenge[] = [
  {
...
const D3_CHALLENGES: CircuitChallenge[] = [
  {
    description:
      "Thevenin equivalent: find Vth and Rth for a complex two-terminal network.",
    components: [
      "Vs (10V)",
      "R1 (4Ω)",
      "R2 (6Ω)",
  ...
  {
    description:
      "Op-amp inverting amplifier: Rin, feedback resistor Rf, virtual ground.",
  ...
  {
    description:
      "LC resonant circuit: inductor and capacitor at resonant frequency.",
    components: [
      "AC Source",
      "Inductor (L)",
      "Capacitor (C)",
...
const OHM_PROBLEMS: OhmProblem[] = [
  ...
  },
  {
    ...
    correct: 2,
    explanation:
      "I = V/R = 6/12 = 0.5A (each branch has same voltage in parallel)",
    circuitType: "Parallel",
  },
  {
...
const SAFETY_SCENARIOS: SafetyScenario[] = [
  {
    situation:
      "A frayed electrical wire is exposed on a lamp cord in the living room.",
    hazardLevel: "HIGH",
    correctAction: 1,
    actions: [
    ...
    explanation:
      "Frayed wires cause fires and electrocution. Replace immediately.",
  },
  {
    situation:
      "You are plugging in 6 high-wattage devices into one power strip.",
    hazardLevel: "HIGH",
    correctAction: 2,
    actions: [
    ...
    explanation:
      "Overloaded sockets overheat and cause fires. Spread load to separate circuits.",
  },
  {
    situation:
      "Your hands are wet after washing dishes and you need to use the microwave.",
    ...
    explanation:
      "Water conducts electricity. Wet hands near electrical switches risk electrocution.",
  },
  {
    ...
    explanation:
      "Cables under carpet overheat, fray, and cause fires without being visible.",
  },
  {
    ...
    explanation:
      "Bathrooms require ground fault circuit interrupters (GFCI) to prevent electrocution.",
  },
  {
    situation:
      "You smell burning plastic from an electrical appliance while it's running.",
    hazardLevel: "CRITICAL",
    correctAction: 0,
    actions: [
    ...
    explanation:
      "Burning smell indicates wiring failure or overheating — risk of fire and electrocution.",
  },
  {
    situation:
      "A child is about to insert a metal object into an unused power socket.",
    hazardLevel: "CRITICAL",
    correctAction: 1,
    actions: [
    ...
    explanation:
      "Metal in live sockets causes electrocution. Socket covers are essential child safety.",
  },
  {
    ...
    explanation:
      "Wet equipment shorts circuit. Must be properly dried and inspected before reuse.",
  },
  {
    situation:
      "You want to hang a picture and need to drill near an electrical outlet.",
    hazardLevel: "MEDIUM",
    correctAction: 1,
    actions: [
    ...
    explanation:
      "Drilling into hidden wires causes shock. Always turn off the circuit and check for cables.",
  },
  {
    situation:
      "Your electricity bill shows unusually high usage and the meter spins fast with all appliances off.",
    hazardLevel: "MEDIUM",
    correctAction: 2,
    actions: [
      "Ignore it",
    ...
    explanation:
      "Abnormal consumption may indicate a fault, leakage, or meter error requiring professional check.",
  },
  {
    situation:
      "The circuit breaker trips repeatedly when you turn on the air conditioner.",
    hazardLevel: "HIGH",
    correctAction: 1,
    actions: [
    ...
    explanation:
      "Repeated tripping means the circuit is overloaded or the appliance has a fault — needs inspection.",
  },
  {
    situation:
      "An electrical fire starts in the kitchen. There's a fire extinguisher nearby.",
    hazardLevel: "CRITICAL",
    correctAction: 0,
    actions: [
    ...
    explanation:
      "Never use water on electrical fires. CO2 or dry powder extinguishers are safe for electrical fires.",
  },
];
...
export default function Electricity({ config, onGameEnd }: Props) {
  const gameId = config.gameId;
  if (gameId === "ohms-law")
    return <OhmsLaw config={config} onGameEnd={onGameEnd} />;
  if (gameId === "electrical-safety")
    return <ElectricalSafety config={config} onGameEnd={onGameEnd} />;
  return <CircuitBuilder config={config} onGameEnd={onGameEnd} />;
}

// ============================================================================
// GAME 1 — Circuit Builder
// ============================================================================
function CircuitBuilder({ config, onGameEnd }: Props) {
  const challenges =
    config.difficulty === 1
      ? D1_CHALLENGES
      : config.difficulty === 2
        ? D2_CHALLENGES
        : D3_CHALLENGES;
  const [phase, setPhase] = useState<"start" | "wiring" | "ohm" | "feedback">(
    "start",
  );
  ...
  const [circuitLit, setCircuitLit] = useState(false);
  ...
  function handleComponentClick(compIdx: number) {
  ...
  function handleOhmAnswer(idx: number) {
  ...
  const wireColor =
    flash === "correct" ? "#10b981" : flash === "wrong" ? "#f43f5e" : "#f59e0b";
  ...
  return (
    ...
    >
        ...
        >
          <div className="glass-card rounded-2xl p-10 text-center max-w-md w-full">
            ...
            <p className="text-muted-foreground mb-2 text-sm">
              Click components in the correct wiring order to complete each
              circuit.
            </p>
            <GlowButton
              variant="primary"
              ...
              >
                ...
                <div className="flex items-center gap-1 mb-4 flex-wrap">
                  {Array.from({ length: slotCount }).map((_, i) => {
                    const slotComp =
                      activeSlots[i] !== null
                        ? currentC.components[activeSlots[i] as number]
                        : null;
                    return (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {circuitLit
                      ? "Circuit complete!"
                      : "Wire the circuit correctly to light the bulb."}
                  </span>
                </div>
                {feedbackMsg && (
                  <motion.div
      ...
      {phase === "ohm" && currentC.ohmQuestion && (
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="flex-1 flex items-center justify-center"
        >
          <div className="glass-card rounded-2xl p-8 max-w-lg w-full border-2 border-[#f59e0b]/40">
            <Zap className="h-10 w-10 mb-4" style={{ color: "#f59e0b" }} />
            <p
              className="text-xs uppercase tracking-widest mb-2 text-muted-foreground"
              style={{ fontFamily: "'Orbitron',sans-serif" }}
            >
            ...
            <p className="text-base font-semibold text-foreground mb-5">
              {currentC.ohmQuestion.text}
            </p>
            <div className="grid grid-cols-2 gap-2">
              {currentC.ohmQuestion.answers.map((ans, i) => (
                <button
                  key={`ohm-${i}`}
                  type="button"
...
// ============================================================================
// GAME 2 — Ohm's Law Calculator
// ============================================================================
function OhmsLaw({ config, onGameEnd }: Props) {
  const totalQ =
      ...
      {phase === "start" && (
        ...
        >
          <div className="glass-card rounded-2xl p-10 text-center max-w-md w-full">
            ...
            <p className="text-muted-foreground mb-2 text-sm">
              Circuit diagrams show 2 known values. Apply V=IR and power
              formulas to find the missing value. {problems.length} problems.
            </p>
            <GlowButton
              variant="primary"
              size="lg"
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                >
                  {currentP.circuitType}
                </span>
                <span className="text-xs text-muted-foreground">
                  Problem {pIdx + 1}/{problems.length}
                </span>
              </div>
              <p className="text-sm font-semibold text-foreground mb-2">
                {currentP.circuitDesc}
              </p>
              <div className="glass rounded-lg p-3 border border-border/20 mb-3">
                <p className="text-xs text-muted-foreground mb-1">
                  Known Values:{" "}
                  <span className="font-mono text-foreground">
                    {currentP.knownValues}
                  </span>
                </p>
...
// ============================================================================
// GAME 3 — Electrical Safety
// ============================================================================
function ElectricalSafety({ config, onGameEnd }: Props) {
  const [phase, setPhase] = useState<"start" | "playing" | "feedback">("start");
  ...
  return (
    ...
    >
      <div className="game-hud flex items-center justify-between gap-3 shrink-0">
        ...
        <span className="text-xs text-muted-foreground">
          {sIdx + 1}/{SAFETY_SCENARIOS.length}
        </span>
        <div className="flex items-center gap-1">
          {Array.from({ length: config.livesCount }).map((_, i) => (
            <Heart
      ...
      {phase === "start" && (
        ...
        >
          <div className="glass-card rounded-2xl p-10 text-center max-w-md w-full">
            ...
            <p className="text-muted-foreground mb-2 text-sm">
              12 household electrical hazard scenarios. Identify the danger
              level and choose the correct action.
            </p>
            <GlowButton
              variant="primary"
              size="lg"
