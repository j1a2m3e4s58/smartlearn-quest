...
// ─── Game 1: Gear Ratio Lab ──────────────────────────────────────────────────

type GearSize = "small" | "medium" | "large";
type GearSlotId = "g1" | "g2" | "g3" | "output";

const GEAR_TEETH: Record<GearSize, number> = {
  small: 12,
  medium: 24,
  large: 48,
};
...
const GEAR_COLORS: Record<GearSize, string> = {
  small: "#00f5ff",
  medium: "#7c3aed",
  large: "#f59e0b",
};

interface GearChallenge {
  title: string;
  description: string;
  driverSize: GearSize;
  driverRPM: number;
  targetRPM: number;
  targetDirection: "same" | "opposite";
  slots: GearSlotId[];
  requiredSizes: GearSize[];
  explanation: string;
}

const GEAR_CHALLENGES: Record<1 | 2 | 3, GearChallenge[]> = {
  1: [
    {
      title: "Simple Speed Reduction",
      description:
        "The driving gear spins at 200 RPM. Select one intermediate gear to reduce speed to 100 RPM.",
      driverSize: "small",
      driverRPM: 200,
      targetRPM: 100,
      ...
      explanation:
        "Small(12T) driving Medium(24T): ratio = 12/24 = 0.5. 200 x 0.5 = 100 RPM. Opposite direction.",
    },
    {
      title: "Speed Increase",
      description:
        "Driver at 100 RPM. Select output gear to achieve 200 RPM (speed up).",
      driverSize: "medium",
      driverRPM: 100,
      targetRPM: 200,
      targetDirection: "opposite",
      ...
      requiredSizes: ["small"],
      explanation:
        "Medium(24T) driving Small(12T): ratio = 24/12 = 2. 100 x 2 = 200 RPM. Gear ratio > 1 = speed increase.",
    },
  ],
  2: [
    {
      ...
      description:
        "Driver at 400 RPM. Use two intermediate gears to achieve 50 RPM output.",
      driverSize: "small",
      driverRPM: 400,
      targetRPM: 50,
      ...
      explanation:
        "Small to Medium: 400x(12/24)=200 RPM. Medium to Large: 200x(24/48)=50 RPM. Even stages = same final direction.",
    },
    {
      title: "High Torque System",
      description:
        "Driver at 300 RPM. Reduce to approximately 75 RPM for higher torque output.",
      driverSize: "small",
      driverRPM: 300,
      targetRPM: 75,
      targetDirection: "same",
      ...
      requiredSizes: ["medium", "large"],
      explanation:
        "Small to Medium: 300x(12/24)=150. Medium to Large: 150x(24/48)=75. Two stages = same direction.",
    },
  ],
  3: [
    {
      ...
      description:
        "Complex gear train. Driver 480 RPM, target output near 30 RPM.",
      driverSize: "small",
      driverRPM: 480,
      targetRPM: 30,
      targetDirection: "opposite",
      ...
      explanation:
        "Three reductions: x(12/24)x(12/24)x(12/48) would overshoot. Correct: small(12)>medium(24)>large(48) gives 480x(12/24)x(24/48)=60 then try variants. Concept: odd stages = opposite direction.",
    },
  ],
};
...
function computeOutputRPM(
  driverRPM: number,
  driverSize: GearSize,
  slots: GearSlot[],
): { rpm: number; valid: boolean } {
...
function GearRatioLabGame({
  ...
  function handleSelectGear(slotId: GearSlotId, size: GearSize) {
    if (submitted) return;
      ...
      >
        ...
        >
          ...
          <p className="text-muted-foreground mb-2 text-sm">
            Select gear sizes to build gear trains that achieve a target output
            speed. Formula: Output RPM = Input RPM x (Driver Teeth / Driven
            Teeth).
          </p>
          <p className="text-muted-foreground text-xs mb-6">
            Small = 12T, Medium = 24T, Large = 48T.
          </p>
          <GlowButton
            variant="primary"
            size="lg"
      ...
      <div className="glass-card rounded-xl p-3 shrink-0 border border-[#f59e0b]/30">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          ...
          <span className="font-bold text-[#10b981]">
            {challenge.targetRPM} RPM
          </span>{" "}
          | Direction:{" "}
          <span className="font-bold text-[#7c3aed]">
            {challenge.targetDirection} as driver
          </span>
        </p>
      </div>
      <div
        ...
        {slots.map((slot) => (
          <div key={slot.id} className="flex items-center gap-3">
            ...
            <div className="flex gap-2">
              {(["small", "medium", "large"] as GearSize[]).map((size) => (
                <motion.button
                  type="button"
                  key={size}
      ...
      <div className="shrink-0 flex items-center justify-between">
        <p className="text-sm">
          ...
          <span
            ...
            style={{
              color:
                previewValid && Math.abs(previewRPM - challenge.targetRPM) <= 5
                  ? "#10b981"
                  : "#f59e0b",
            }}
          >
            {previewValid ? `${previewRPM} RPM` : "--"}
          </span>{" "}
          / Target:{" "}
...
// ─── Game 2: Force Machine (Simple Machines) ───────────────────────────────
...
interface MachineProblem {
  ...
  planeLength?: number;
  planeHeight?: number;
  correctFormula: FormulaChoice;
  correctMA: number;
  formulaOptions: FormulaChoice[];
  explanation: string;
  diagramDesc: string;
}

const FORMULA_LABELS: Record<FormulaChoice, string> = {
...
const MACHINE_PROBLEMS: Record<1 | 2 | 3, MachineProblem[]> = {
  1: [
    {
      id: "m1",
      machineType: "lever",
      ...
      explanation:
        "MA = Load/Effort = 60/20 = 3. A mechanical advantage of 3 means you lift 3x more than you push.",
      diagramDesc:
        "Fulcrum at center. Effort arm = 150cm on left. Load arm = 50cm on right. Load = 60N. Effort = 20N.",
    },
    {
      id: "m2",
      ...
      explanation:
        "MA = Length/Height = 10/2 = 5. A longer ramp reduces the force needed to lift the same load.",
      diagramDesc:
        "Ramp length = 10m. Ramp height = 2m. Load weight = 500N. Effort to push = 100N along slope.",
    },
  ],
  2: [
    {
      ...
      explanation:
        "MA = Load/Effort = 100/50 = 2. A movable pulley halves the effort needed to lift a load.",
      diagramDesc:
        "Fixed pulley at top (changes direction). Movable pulley attached to 100N load. Effort = 50N upward on rope.",
    },
    {
      id: "m4",
      ...
      explanation:
        "MA = Wheel Radius / Axle Radius = 30/5 = 6. The large wheel multiplies torque transmitted to the small axle.",
      diagramDesc:
        "Wheel radius = 30cm. Axle radius = 5cm. Effort applied at wheel rim. Load resists at axle.",
    },
    {
      id: "m5",
      machineType: "lever",
      ...
      explanation:
        "MA = Effort Distance / Load Distance = 100/20 = 5. Longer effort arm means less force needed.",
      diagramDesc:
        "Fulcrum at one end. Load = 200N at 20cm from fulcrum. Effort = 40N at 100cm from fulcrum.",
    },
  ],
  3: [
    {
      ...
      explanation:
        "MA = 100/25 = 4. Block and tackle with 4 rope sections supporting the load gives MA = 4.",
      diagramDesc:
        "4-rope block and tackle. Load = 100N. Effort = 25N. 4 rope sections support the movable block.",
    },
    {
      id: "m7",
      machineType: "inclined_plane",
      ...
      explanation:
        "MA = L/H = 20/1 = 20. A screw is an inclined plane wrapped around a cylinder — very high MA.",
      diagramDesc:
        "Thread pitch = 1mm (height). Circumference = 20mm (length). Load = 600N. Effort = 30N at handle.",
    },
    {
      id: "m8",
      machineType: "wheel_axle",
      ...
      explanation:
        "MA = 40/8 = 5. The winch handle (large radius) turns the drum (small axle) with 5x mechanical advantage.",
      diagramDesc:
        "Winch handle radius = 40cm. Drum axle radius = 8cm. Rope winds on drum to lift load.",
    },
    {
      id: "m9",
      machineType: "lever",
      ...
      explanation:
        "MA = Effort Distance / Load Distance = 120/30 = 4. Wheelbarrow handles far from axle give 4x advantage.",
      diagramDesc:
        "Axle at wheel end. Load at 30cm from axle. Handles at 120cm from axle. Effort = 60N. Load = 240N.",
    },
  ],
};
...
function ForceMachineGame({
  ...
  const [selectedFormula, setSelectedFormula] = useState<FormulaChoice | null>(
    null,
  );
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Simple Machines Analyzer
          </h2>
          <p className="text-muted-foreground mb-2 text-sm">
            Analyze lever, pulley, inclined plane, and wheel-and-axle diagrams.
            Select the correct formula and calculate the mechanical advantage.
          </p>
          <p className="text-muted-foreground text-xs mb-6">
            Mechanical Advantage (MA) measures how much a machine multiplies
            force. Higher MA = less effort needed.
          </p>
          <GlowButton
            variant="primary"
            size="lg"
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {MACHINE_LABELS[problem.machineType].toUpperCase()} DIAGRAM
        </p>
        <div
        ...
        >
          <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
            ...
            {problem.machineType === "inclined_plane" && (
              <>
                <polygon
            ...
            {problem.machineType === "wheel_axle" && (
              <>
                <circle
                  cx="50%"
                  cy="50%"
      ...
      <div className="shrink-0">
        ...
        <div className="grid grid-cols-2 gap-2">
          {problem.formulaOptions.map((formula) => (
            <motion.button
              type="button"
              key={formula}
              ...
              style={{
                ...
                backgroundColor:
                  selectedFormula === formula ? `${color}20` : "transparent",
          ...
          {maOptions.map((ma) => (
            <motion.button
              ...
              style={{
                ...
                backgroundColor:
                  selectedMA === ma ? `${color}20` : "transparent",
            ...
            <p className="text-sm">
              {result ? (
                ...
                <span className="text-[#f43f5e]">
                  Incorrect. Check the diagram values and formula again.
                </span>
              )}
            </p>
          </motion.div>
...
// ─── Game 3: Bridge Builder (Structural Analysis) ─────────────────────────

type BridgeType = "beam" | "arch" | "suspension" | "truss";
...
const BRIDGE_QUESTIONS: Record<1 | 2 | 3, BridgeQuestion[]> = {
  1: [
    {
      id: "bq1",
      question:
        "Which bridge type uses triangles to distribute load across a rigid framework?",
      options: [
        "Beam Bridge",
        "Arch Bridge",
        "Truss Bridge",
      ...
      bridgeType: "truss",
      explanation:
        "Truss bridges use triangular units because triangles are the only rigid polygon — they cannot deform under load.",
    },
    {
      id: "bq2",
      question:
        "In a suspension bridge, where is the maximum TENSION force experienced?",
      options: [
      ...
      explanation:
        "The main suspension cables carry maximum tension, stretching between towers. Cables excel in tension, not compression.",
    },
    {
      id: "bq3",
      ...
      bridgeType: "arch",
      explanation:
        "An arch converts vertical loads into outward horizontal thrust through its curved shape. The abutments resist this thrust.",
    },
  ],
  2: [
    {
      id: "bq4",
      question:
        "A 100N load is placed at the center of a simple beam bridge 10m long supported at both ends. What is the reaction force at each support?",
      options: ["25N each", "50N each", "100N each", "75N and 25N"],
      correctIdx: 1,
      bridgeType: "beam",
      explanation:
        "By symmetry, each support carries half the load. Reaction = 100N / 2 = 50N each end.",
    },
    {
      id: "bq5",
      question:
        "Which part of a truss bridge member experiences COMPRESSION when carrying a downward load?",
      options: [
      ...
      bridgeType: "truss",
      explanation:
        "The top chord of a truss is in compression under vertical loading, while the bottom chord is in tension.",
    },
    {
      id: "bq6",
      question:
        "In a suspension bridge, the deck is hung from vertical cables called hangers. What force do the hangers experience?",
      options: ["Compression", "Tension", "Shear", "Torsion"],
      correctIdx: 1,
      bridgeType: "suspension",
      explanation:
        "Hanger cables are in pure tension — they stretch under the weight of the deck they support.",
    },
    {
      id: "bq7",
      question:
        "Which bridge type is most efficient for very short spans (up to 20m) due to simplicity?",
      options: [
        "Truss Bridge",
        "Arch Bridge",
        "Beam Bridge",
      ...
      bridgeType: "beam",
      explanation:
        "Beam bridges are simplest and most cost-effective for short spans. Complexity increases cost for longer spans.",
    },
  ],
  3: [
    {
      id: "bq8",
      question:
        "A truss bridge has a 200N load applied at a single joint. There are 4 members meeting at that joint. What does a force balance require at the joint?",
      options: [
      ...
      bridgeType: "truss",
      explanation:
        "Joint equilibrium: sum of all horizontal forces = 0 AND sum of all vertical forces = 0. This is the method of joints.",
    },
    {
      id: "bq9",
      question:
        "Why are suspension bridges preferred over beam bridges for very long spans (>500m)?",
      options: [
      ...
      bridgeType: "suspension",
      explanation:
        "High-strength steel cables are far lighter and more material-efficient than the massive beams needed for equivalent long spans.",
    },
    {
      id: "bq10",
      question:
        "An arch bridge has a horizontal thrust of 500kN at its abutments. The arch rises 8m and spans 40m. What structural feature resists this outward thrust?",
      options: [
      ...
      bridgeType: "arch",
      explanation:
        "Arch abutments must be anchored into solid rock or massive concrete to resist the horizontal thrust without sliding outward.",
    },
  ],
};
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p className="text-muted-foreground mb-2 text-sm">
            Analyze four bridge types — Beam, Arch, Suspension, and Truss.
            Answer structural engineering questions based on force principles.
          </p>
          <p className="text-muted-foreground text-xs mb-6">
            Each question tests real structural concepts: compression, tension,
            load distribution, reaction forces, and design principles.
          </p>
          <div className="grid grid-cols-2 gap-2 mb-6">
            {(["beam", "arch", "suspension", "truss"] as BridgeType[]).map(
              (bt) => (
                <div
                  key={bt}
    ...
    >
      ...
      >
        <p
          ...
          style={{ color, fontFamily: "'Orbitron', sans-serif" }}
        >
          BRIDGE DIAGRAM — {BRIDGE_LABELS[q.bridgeType].toUpperCase()}
        </p>
        <div
...
export default function MechanicalEngineering({ config, onGameEnd }: Props) {
  if (config.gameId === "force-machine")
    return <ForceMachineGame config={config} onGameEnd={onGameEnd} />;
  if (config.gameId === "bridge-builder")
    return <BridgeBuilderGame config={config} onGameEnd={onGameEnd} />;
  return <GearRatioLabGame config={config} onGameEnd={onGameEnd} />;
}
